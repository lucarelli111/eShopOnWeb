...
#
# Usage:
#   ./build-push-acr.sh [tag] [components...]
#   
#   tag: Image tag (default: latest)
#   components: web, api, traffic (default: all)
#
# Examples:
#   ./build-push-acr.sh                    # Build all with 'latest' tag
#   ./build-push-acr.sh v1.2               # Build all with 'v1.2' tag
#   ./build-push-acr.sh latest web traffic # Build only web and traffic
#   ./build-push-acr.sh v1.0 api           # Build only api with 'v1.0' tag
#
set -e
...
# Parse which components to build
BUILD_WEB=false
BUILD_API=false
BUILD_TRAFFIC=false

# If no components specified after tag, build all
if [ $# -le 1 ]; then
  BUILD_WEB=true
  BUILD_API=true
  BUILD_TRAFFIC=true
  ...
  for component in "$@"; do
    case "$component" in
      ...
      *)
        ...
        echo "Valid components: web, api, traffic"
        exit 1
        ;;
    esac
  done
...
[ "$BUILD_WEB" = true ] && echo "  - Web Application"
[ "$BUILD_API" = true ] && echo "  - Public API"
[ "$BUILD_TRAFFIC" = true ] && echo "  - Traffic Simulator"
